      ...
      weirdNoise(seed + 404)
{
    ...
    biomes.push_back(Biome("Badlands", {0.8f, 0.4f, 0.2f}, 20.0, 60.0, 8, 0.6, 2.0, 0.7, 0.9, 0.25, 0.4, 0.7, 0.5));
...
BiomeManager::ClimatePoint BiomeManager::getClimate(glm::vec3 direction)
{
    ...
    double tNoise = tempNoise.noise(p.x * 0.01, p.y * 0.01, p.z * 0.01);
    double temperature = tempBase * 0.85 + tNoise * 0.15;

    double contRaw = fbm3D(direction * 1.5f, 12, 0.55, contNoise);

    double cont;
    if (contRaw > 0)
    {
        cont = std::pow(contRaw, 0.2);
    }
    else
    {
        cont = -std::pow(std::abs(contRaw), 0.2);
    }
    cont -= 0.1;

    double mRaw = (moistNoise.noise(p.x * 2.5, p.y * 2.5, p.z * 2.5) + 1.0) * 0.5;
    double moisture = 3.0 * mRaw * mRaw - 2.0 * mRaw * mRaw * mRaw;

    double wRaw = (weirdNoise.noise(p.x * 3.0, p.y * 3.0, p.z * 3.0) + 1.0) * 0.5;
    double weirdness = std::pow(wRaw, 3.0);

    double eRaw = (erosionNoise.noise(p.x * 2.0, p.y * 2.0, p.z * 2.0) + 1.0) * 0.5;
    double erosion = std::pow(eRaw, 1.2);
...
Biome BiomeManager::getBiomeAt(glm::vec3 direction)
{
    ...
    for (auto &biome : biomes)
    {
        ...
        double dy = (climate.moisture - biome.moisture) * 1.5;
        double dz = (climate.continentalness - biome.continentalness) * 5.0;
        double de = (climate.erosion - biome.erosion) * 3.0;
        double dw = (climate.weirdness - biome.weirdness) * 10.0;
